...
exports.getConversations = async (req, res) => {
  try {
    const conversations = await Conversation.find({ participants: req.user._id })
      .populate("participants", "name username email avatarColor isOnline lastSeen")
    ...
    const withDetails = await Promise.all(
      conversations.map(async (conv) => {
        ...
        const connection = await Connection.findOne({
          $or: [
            { requester: req.user._id, recipient: otherUser?._id },
            { requester: otherUser?._id, recipient: req.user._id },
          ],
        });
    ...
    res.status(200).json({ success: true, conversations: withDetails });
  } catch (error) {
  ...
  try {
    ...
    if (!conversation) {
      ...
      conversation = await conversation.populate(
        "participants",
        "name username email avatarColor isOnline lastSeen"
      );
    }
